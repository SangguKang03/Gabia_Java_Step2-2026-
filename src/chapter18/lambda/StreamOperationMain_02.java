...
public class StreamOperationMain_02 {
	...
	public static void main(String[] args) {
		...
		list.stream().filter(lists -> lists.length() >= 5).forEach(System.out::println);
		...
		Arrays.stream(arr).sorted().filter(num -> num % 2 == 0).forEach(System.out::println);
		
		// 배수(filter())) &&
		System.out.println();
		System.out.println("2의 배수도 아니고, 3의 배수도 아닌 숫자 출력");
		Arrays.stream(arr).sorted().filter(num -> num %2 !=0 && num %3 != 0).forEach(System.out::println);
